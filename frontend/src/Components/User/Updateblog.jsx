...
const UpdateBlog = () => {
  ...
  const fetchBlog = async () => {
    try {
      ...
      console.log('Fetched blog imageUrl:', blog.imageUrl); 
      setFormData({
        title: blog.title,
      ...
      // Fix based on what blog.imageUrl contains
      const cleanImagePath = blog.imageUrl?.includes('uploads')
  ...
  return (
    <div className="max-w-4xl mx-auto mt-12 mb-13 p-6 bg-white rounded-xl shadow-md border border-blue-200">
      <h2 className="text-2xl font-bold mb-6 text-[#005A9C] text-center">Update Blog</h2>
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data" className="space-y-5">
        ...
        <div className="text-center">
          <button
            type="submit"
            disabled={loading}
            className={`bg-[#005A9C] text-white px-6 py-2 rounded-lg transition-all duration-200 ${
